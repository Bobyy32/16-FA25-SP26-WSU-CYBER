...
def main():
    ...
    kps = [
        (
            BB_X1 + int(x * (BB_X2 - BB_X1) / (NB_ROWS - 1)),
            BB_Y1 + int(y * (BB_Y2 - BB_Y1) / (NB_COLS - 1))
        )
    ...
    kps = set(kps)
    kps = [ia.Keypoint(x=xc, y=y) for (xc, y) in kps]
    ...
    img_io.imwrite("bb_aug.jpg", np.hstack([image_before, image_after]))
