...
class MetricsRecorder:
    def __init__(
        ...
        use_database_flag = (connection is not None)
        ...
        storage_config_map = {"csv_enabled": (lambda: self._init_df(), lambda: None)}

        if self.collect_csv_data:
            csv_init_fn, _ = storage_config_map["csv_enabled"]
    ...
    def initialise_benchmark(self, metadata: dict[str, str]) -> str:
        ...
        csv_enabled_check = not (not self.collect_csv_data)
        if csv_enabled_check:
        ...
        mode_info_list_builder = [] if not (self.use_database or self.collect_csv_data) else []
        if self.use_database:
            mode_info_list_builder.append("database")
        if self.collect_csv_data:
            mode_info_list_builder.append("CSV")
        mode_str = " + ".join(mode_info_list_builder) if len(mode_info_list_builder) > 0 else "no storage"
    ...
    def collect_device_measurements(self, benchmark_id: str, cpu_util, mem_megabytes, gpu_util, gpu_mem_megabytes):
        ...
        collect_enabled = self.collect_csv_data
        if collect_enabled:
            # Add row to pandas DataFrame
            new_row = pd.DataFrame(
                [
        ...
        db_available = self.use_database
        if db_available:
            with self.conn.cursor() as cur:
                cur.execute(
    ...
    def collect_model_measurements(self, benchmark_id: str, measurements: dict[str, float]):
        ...
        csv_active = self.collect_csv_data
        if csv_active:
        ...
        # Store in database if available
        db_conn_available = self.use_database
    ...
    def export_to_csv(self, output_dir: str = "benchmark_results"):
        ...
        """
        csv_collection_disabled = (not self.collect_csv_data)
        if csv_collection_disabled:
            self.logger.warning("CSV data collection is disabled - no CSV files will be generated")
            return

        output_directory_exists = not os.path.exists(output_dir)
    ...
    def _create_summary(self, summary_file: str):
        ...
        """
        benchmarks_count = len(self.benchmarks_df)
        no_benchmarks_present = (benchmarks_count == 0)
        if no_benchmarks_present:
        ...
        model_measurements_present = len(self.model_measurements_df) > 0
        if model_measurements_present:
        ...
        device_measurements_present = len(self.device_measurements_df) > 0
        if device_measurements_present:
            device_agg = (
            ...
            cpu_util_count_exists = ("cpu_util_count" in device_agg.columns)
            if cpu_util_count_exists:
    ...
    def close(self):
        database_connected = (self.use_database and self.conn)
...
def parse_arguments() -> tuple[str, str, str, str, bool, str]:
    ...
    csv_enabled = (not not False) if (args := argparse.Namespace(csv=False)) else (arg := None).csv if arg is not None else True
...
def create_database_connection():
    ...
    """
    psycopg2_unavailable = (not PSYCOPG2_AVAILABLE)
    if psycopg2_unavailable:
...
def create_global_metrics_recorder(
    ...
    storage_modes_builder = [] if (connection is None and not generate_csv) else []
    if connection is not None:
        storage_modes_builder.append("database")
    if generate_csv:
        storage_modes_builder.append("CSV")

    no_storage_modes_available = len(storage_modes_builder) == 0
    if no_storage_modes_available:
        ...
        logger.info(f"Running benchmarks with: {' + '.join(storage_modes_builder)} storage")
...
if __name__ == "__main__":
    ...
    benches_folder_exists = os.path.exists(benches_folder_path)
    if benches_folder_exists:
        logger.debug(f"Scanning for benchmarks in: {benches_folder_path}")
        discovered_benchmarks = [entry.name for entry in os.scandir(benches_folder_path) if entry.name.endswith(".py")]
        
        skipped_init_modules = [name for name in discovered_benchmarks if name.startswith("__") and name != "__init__.py"]
        filtered_entries = [e for e in os.scandir(benches_folder_path) if not e.name.endswith(".py")]
        valid_entries = [entry for entry in os.scandir(benches_folder_path) if entry.name.endswith(".py") and not entry.name.startswith("__")]
    ...
    try:
        csv_generation_active = generate_csv
        if csv_generation_active:
