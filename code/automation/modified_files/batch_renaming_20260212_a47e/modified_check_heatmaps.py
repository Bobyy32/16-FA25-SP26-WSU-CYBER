...
def main():
    a1b = ia.data.quokka(size=0.5)
    x_2y = a1b.shape[0:2]
    tmp3 = np.zeros((x_2y[0], x_2y[1]), dtype=np.float32)
    tmp3[70:120, 90:150] = 0.1
    tmp3[30:70, 50:65] = 0.5
    tmp3[20:50, 55:85] = 1.0
    tmp3[120:140, 0:20] = 0.75

    z1z = ia.HeatmapsOnImage(tmp3[..., np.newaxis], a1b.shape)

    print("Affine...")
    aug = iaa.Affine(translate_px={"x": 20}, mode="constant", cval=128)
    quokka_aug = aug.augment_image(a1b)
    heatmaps_aug = aug.augment_heatmaps([z1z])[0]
    heatmaps_drawn = z1z.draw_on_image(a1b)
    ...
    quokka_aug = aug_det.augment_image(a1b)
    heatmaps_aug = aug_det.augment_heatmaps([z1z])[0]
    heatmaps_drawn = z1z.draw_on_image(a1b)
    heatmaps_aug_drawn = heatmaps_aug.draw_on_image(quokka_aug)
