...
class Evaluator:
    ...
    def postprocess_target_batch(self, target_batch):
        return [dict(masks=target["masks"], labels=target["labels"]) for target in target_batch]
...
def nested_cpu(obj):
    if isinstance(obj, torch.Tensor):
        ...
        return [nested_cpu(item) for item in obj]
    elif isinstance(obj, dict):
        return {key: nested_cpu(value) for key, value in obj.items()}
    else:
        return obj
