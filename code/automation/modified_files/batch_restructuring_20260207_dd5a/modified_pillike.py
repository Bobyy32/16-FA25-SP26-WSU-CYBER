...
def _ensure_valid_shape(image, func_name):
    ...
    result = image[:, :, 0] if is_hw1 else image
    assert (
        result.ndim == 2
        or (result.ndim == 3 and result.shape[-1] in [3, 4])
    ), (
        "Can apply %s only to images of "
        "shape (H, W) or (H, W, 1) or (H, W, 3) or (H, W, 4). "
        "Got shape %s." % (func_name, result.shape,))
    return result, is_hw1
...
class _FilterBase(meta.Augmenter):
    """Base class for all filter augmenters."""
    def __init__(self, seed=None, name=None, random_state="deprecated", 
                 deterministic="deprecated"):
        super(_FilterBase, self).__init__(
            seed=seed, name=name, random_state=random_state, 
...
class FilterBlur(_FilterBase):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        return [filter_blur(image) for image in images]
...
class FilterSmooth(_FilterBase):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        return [filter_smooth(image) for image in images]
...
class FilterSmoothMore(_FilterBase):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        return [filter_smooth_more(image) for image in images]
...
class FilterEdgeEnhance(_FilterBase):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        return [filter_edge_enhance(image) for image in images]
...
class FilterEdgeEnhanceMore(_FilterBase):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        return [filter_edge_enhance_more(image) for image in images]
...
class FilterFindEdges(_FilterBase):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        return [filter_find_edges(image) for image in images]


class FilterContour(_FilterBase):
    """Apply a contour detection filter kernel to the image."""
    def __init__(self, seed=None, name=None, random_state="deprecated", 
                 deterministic="deprecated"):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        return [filter_contour(image) for image in images]
...
class FilterEmboss(_FilterBase):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        return [filter_emboss(image) for image in images]
...
class FilterSharpen(_FilterBase):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        return [filter_sharpen(image) for image in images]
...
class FilterDetail(_FilterBase):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        return [filter_detail(image) for image in images]
