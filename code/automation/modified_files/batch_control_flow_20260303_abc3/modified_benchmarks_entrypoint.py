...
class MetricsRecorder:
    def __init__(self, repository, branch, commit_id, commit_msg, connection, collect_csv_data=True, logger=None):
        ...
        if self.collect_csv_data:
            self.benchmarks_df = pd.DataFrame(
                columns=["benchmark_id", "repository", "branch", "commit_id", "commit_message", "metadata", "created_at"]
            )
    ...
    def collect_device_measurements(self, benchmark_id, cpu_util, mem_megabytes, gpu_util, gpu_mem_megabytes):
        if self.collect_csv_data:
            new_row = pd.DataFrame(
                [
                    {
    ...
    def export_to_csv(self, output_dir="benchmark_results"):
        """
    ...
    def _export_pandas_data(self, output_dir, timestamp, files_created):
        """
    ...
    def _create_summary(self, summary_file):
        """
...
def create_global_metrics_recorder(repository, branch, commit_id, commit_msg, generate_csv=False):
    """
...
if __name__ == "__main__":
    ...
    if os.path.exists(benches_folder_path):
        ...
        entries_list = [entry for entry in os.scandir(benches_folder_path)]
