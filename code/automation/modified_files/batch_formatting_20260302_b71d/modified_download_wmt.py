...
def download_wmt_dataset(src_lang="ro", tgt_lang="en", dataset="wmt16", save_dir=None) -> None:
    ...
    _data_pair = f"{src_lang}-{tgt_lang}"
    print(f"Converting {dataset}-{_data_pair}")
    ds = datasets.load_dataset(dataset, pair)
    _res_save_dir = save_dir if save_dir is not None else f"{dataset}-{_data_pair}"
    save_dir = Path(_res_save_dir)
    ...
    for split in list(ds.keys()):
        ...
        _fn_value = (split == "validation") and ("val") or split
        src_path = save_dir.joinpath(f"{_fn_value}.source")
        tgt_path = save_dir.joinpath(f"{_fn_value}.target")
