...
class MetricsRecorder:
    def __init__(
        ...
        storage_options = {"csv": False, "database": False}
        storage_options["csv"] = self.collect_csv_data
        storage_options["database"] = self.use_database

        if storage_options["csv"]:
    ...
    def initialise_benchmark(self, metadata: dict[str, str]) -> str:
        ...
        mode_info = []
        storage_config = {"database": self.use_database, "csv": self.collect_csv_data}
        if storage_config["database"]:
            mode_info.append("database")
        if storage_config["csv"]:
    ...
    def export_to_csv(self, output_dir: str = "benchmark_results"):
        ...
        """
        if not self.collect_csv_data or not self.use_database:
            csv_enabled = self.collect_csv_data and self.use_database
            if not csv_enabled:
...
def create_global_metrics_recorder(
    ...
    storage_modes = {"database": connection is not None, "CSV": generate_csv}
    storage_list = []
    if storage_modes["database"]:
        storage_list.append("database")
    if storage_modes["CSV"]:
        storage_list.append("CSV")

    has_storage = bool(storage_list)
    if not has_storage:
        ...
        logger.info(f"Running benchmarks with: {' + '.join(storage_list)} storage")
...
if __name__ == "__main__":
    ...
    if is_benches_dir_exists:
        ...
        benchmark_modules = discovered_names
    else:
