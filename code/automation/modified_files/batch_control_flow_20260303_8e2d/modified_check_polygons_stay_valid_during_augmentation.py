...
def create_random_polygon(height, width, seed):
    ...
    coordinates = rs.rand(nb_points, 2)
    coordinates = (coordinates * 2 - 0.5) # allow coords outside of image plane
    coordinates[:, 0] *= width
    ...
    fresh_seed = rs.randint(ia.SEED_MIN_VALUE, ia.SEED_MAX_VALUE)
    return create_random_polygon(height, width, fresh_seed)
