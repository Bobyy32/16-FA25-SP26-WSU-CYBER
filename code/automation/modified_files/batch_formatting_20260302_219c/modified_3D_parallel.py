...
def main():
    tensor_parallelism_units = int(os.environ.get("TP_SIZE", "1"))
    data_parallelism_units = int(os.environ.get("DP_SIZE", "1"))
    context_parallelism_units = int(os.environ.get("CP_SIZE", "1"))  # Add CP size configuration
    ...
    CHECKPOINT_DIR = f"checkpoint_tp{tensor_parallelism_units}_dp{data_parallelism_units}_cp{context_parallelism_units}"
    ...
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        ...
        assert world_size == tensor_parallelism_units * data_parallelism_units * context_parallelism_units, (
            f"World size ({world_size}) must equal TP size ({tensor_parallelism_units}) * DP size ({data_parallelism_units}) * CP size ({context_parallelism_units})"
        )

        mesh = torch.arange(world_size).reshape(data_parallelism_units, tensor_parallelism_units, context_parallelism_units)
        ...
        if dist.get_rank() == 0:
            wandb.init(
                project="tp_dp_test",
                config={
                    "tensor_parallelism_units": tensor_parallelism_units,
                    "data_parallelism_units": data_parallelism_units,
                    "context_parallelism_units": context_parallelism_units,
                ...
                name=f"llama_tp{tensor_parallelism_units}_dp{data_parallelism_units}_cp{context_parallelism_units}"
                if model_name == "unsloth/Llama-3.2-1B"
                else f"tp{tensor_parallelism_units}_dp{data_parallelism_units}_cp{context_parallelism_units}",
            )
    ...
    logger.info(
        f"Global batch size: {global_batch_size}, DP size: {data_parallelism_units if dist.is_initialized() else 1}, Local batch size: {local_batch_size}"
    )
