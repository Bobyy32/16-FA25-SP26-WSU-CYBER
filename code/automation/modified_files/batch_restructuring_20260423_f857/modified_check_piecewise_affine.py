...
def draw_keypoints_single(keypoint, image, color, size):
    y, x = keypoint.y + keypoint.border, keypoint.x + keypoint.border
    x1 = max(x - size//2, 0)
    x2 = min(x + 1 + size//2, width - 1)
    y1 = max(y - size//2, 0)
    y2 = min(y + 1 + size//2, height - 1)
...
def process_augmentation(aug_det, image, kps):
    img_aug = aug_det.augment_image(image)
    kps_aug = aug_det.augment_keypoints(kps)[0]
    return img_aug
