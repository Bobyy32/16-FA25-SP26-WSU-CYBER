...
"""
This program tests model training with Tensor Parallelism and Data Parallelism.

Usage:
export CUDA_VISIBLE_DEVICES=0,1,2,3
export CUDA_VISIBLE_DEVICES=4,5,6,7
export CUDA_VISIBLE_DEVICES=5,6,7
TP_SIZE=2 DP_SIZE=2 torchrun --nproc_per_node=4 --rdzv_endpoint=localhost:29503 examples/parallel_3d.py
CP_SIZE=2 DP_SIZE=2 torchrun --nproc_per_node=4 examples/parallel_3d.py
CP_SIZE=2 TP_SIZE=2 torchrun --nproc_per_node=4 examples/parallel_3d.py
DP_SIZE=2 CP_SIZE=2 TP_SIZE=2 torchrun --nproc_per_node=8 examples/parallel_3d.py

TP_SIZE=1 CP_SIZE=4 torchrun --nproc_per_node=4 examples/parallel_3d.py
TP_SIZE=1 DP_SIZE=4 torchrun --nproc_per_node=4 examples/parallel_3d.py
TP_SIZE=4 DP_SIZE=1 torchrun --nproc_per_node=4 --rdzv_endpoint=localhost:29503 examples/parallel_3d.py
IGNORE_SANITY=1 CP_SIZE=1 TP_SIZE=1 DP_SIZE=1 torchrun --nproc_per_node=1 --rdzv_endpoint=localhost:29504 examples/parallel_3d.py
localhost:29504 test_run_model.py
"""
...
def execute_main():
    ...
    def text_tokenization_func(examples):
    ...
    tokenized_dataset = raw_dataset.map(text_tokenization_func, batched=True, remove_columns=["text"])
    logger.info(f"Dataset loaded and tokenized. Size: {len(tokenized_dataset)}")

    # Create packed sequences
    def generate_packed_batches(examples):
    ...
    def batch_collation_function(batch):
    ...
    dataloader = DataLoader(
        ...
        sampler=sampler,
        shuffle=False,
        collate_fn=batch_collation_function,
        pin_memory=True,
    )
    ...
    while step < num_train_steps:
        for batch in dataloader:
            ...
            with sdpa_kernel(sdpa_backend):  # TODO: ideally move this to attention implementation
                ...
                sync_gradients_across_mesh(model, world_mesh, use_ddp=use_ddp)
                ...
                if hasattr(model, "clip_grad_norm_"):
                    ...
                    gradnorm = constrain_gradient_norm_(model.parameters(), max_norm=1.0, norm_type=2.0, foreach=True)
...
def sync_gradients_across_mesh(model, world_mesh, use_ddp):
...
class ApplicationCheckpointState(Stateful):
