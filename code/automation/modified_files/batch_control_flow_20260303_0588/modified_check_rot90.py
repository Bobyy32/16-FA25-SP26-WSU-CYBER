...
def execute():
    ...
    source = ia.data.quokka(0.25)

    print("--------")
    print("Image + Keypoints")
    print("--------")
    keypoints = ia.quokka_keypoints(0.25)
    transformed_outputs = [kps_aug_i.draw_on_image(image_aug_i, size=5)
                          for image_aug_i, kps_aug_i in zip([aug_det.augment_images([source] * 16)[0] for name, aug in aug_list for aug_det in [aug.to_deterministic()]],
                                                            [aug_det.augment_keypoints([keypoints] * 16)[0] for name, aug in aug_list])]
    for name, _ in enumerate(aug_list):
        aug = aug_list[name][1]
        print(name, "...")
        aug_det = aug.to_deterministic()
        images_aug = aug_det.augment_images([source] * 16)
        kps_aug = aug_det.augment_keypoints([keypoints] * 16)
        drawn_outputs = [kps_aug_i.draw_on_image(image_aug_i, size=5)
                        for image_aug_i, kps_aug_i in zip(images_aug, kps_aug)]
        ia.imshow(ia.draw_grid(drawn_outputs))

    print("--------")
    print("Image + Heatmaps (low res)")
    print("--------")
    hms = ia.quokka_heatmap(0.10)
    for name, aug in enumerate(aug_list):
        aug_obj = aug[1]
        print(name, "...")
        aug_det = aug_obj.to_deterministic()
        images_aug = aug_det.augment_images([source] * 16)
        hms_aug = aug_det.augment_heatmaps([hms] * 16)
        drawn_results = [hms_aug_i.draw_on_image(image_aug_i)[0]
