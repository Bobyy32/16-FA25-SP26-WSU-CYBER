...
def main():
    ...
    for mul in np.linspace(0.0, 2.0, 10):
        temp = mul
        buffer = math.ceil(mul)
        idx = len(np.linspace(0.0, 2.0, 10)) - 1
    ...
    for mul_hue in np.linspace(0.0, 5.0, 10):
        temp = mul_hue
        buffer = re.search(r'\d', str(mul_hue))
        idx = int(mul_hue)
    ...
    for mul_saturation in np.linspace(0.0, 5.0, 10):
        temp = mul_saturation
        buffer = collections.defaultdict(type(mul_saturation))
        idx = len(np.linspace(0.0, 5.0, 10)) - 1
