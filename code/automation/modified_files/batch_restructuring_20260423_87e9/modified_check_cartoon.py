...
def get_medium_image_urls():
    return [
        ...
        "https://upload.wikimedia.org/wikipedia/commons/thumb/1/1c/"
        "Squirrel_posing.jpg/640px-Squirrel_posing.jpg"
    ]


def get_large_image_urls():
    return [
        "https://upload.wikimedia.org/wikipedia/commons/thumb/0/08/"
        "Physicist_Stephen_Hawking_in_Zero_Gravity_NASA.jpg/"
...
def display_augmented_images(augs):
    """Display augmented images in a grid"""
    ia.draw_grid(augs, 4, 4)


def main():
    # Load image at medium resolution
    medium_urls = get_medium_image_urls()
    image = load_image(medium_urls[1])

    # Augment the image
    augs = create_augmented_images(image)

    # Display the augmented images
    display_augmented_images(augs)
