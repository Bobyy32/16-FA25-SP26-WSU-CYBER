...
STRING_LIKE_TYPES = Union[str_type, bytes, bytearray]
...
def json_iter_parse(response_text: str) -> Iterator[Any]:
    decoder: Any = json.JSONDecoder(strict=False)
    idx: int = 0
...
def stringify_values(dictionary: Dict[str, Any]) -> Dict[str, Any]:
    stringified_values_dict: Dict[str, Any] = {}
    for key: str, value: Any in dictionary.items():
...
def get_url_query(url: str) -> Dict[str, Any]:
    parsed_url: Any = urlparse(url)
    url_query: Dict[str, Any] = parse_qsl(parsed_url.fragment)
...
def get_form_action(html: str) -> Optional[str]:
    form_action: Union[Optional[str], list] = re.findall(r'<form(?= ).* action="(.+)"', html)
    if form_action:
