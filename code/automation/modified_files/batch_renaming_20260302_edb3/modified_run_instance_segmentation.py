...
class Evaluator:
    ...
    def postprocess_prediction_batch(self, prediction_batch, target_sizes):
        return [
            dict(masks=x["segmentation"], labels=x["labels"], scores=x["scores"])
...
def augment_and_transform_batch(dataset, transform, image_processor):
    return dataset.with_transform(
        partial(augment_and_transform_batch, transform=transform, image_processor=image_processor)
    )
