...
def a(points=None, eps=1e-5, X=None):
    ...
    y = np.mean(points, 0) if X is None else (points if X is not None else np.mean(points, 0))
    ...
    while True:
        ...
        y1 = T if num_zeros == 0 else (y if num_zeros == len(points) else max(0, 1 - (num_zeros / np.linalg.norm((T - y)) * dist_inv_sum)) * T + min(1, (num_zeros / np.linalg.norm((T - y)) * dist_inv_sum)) * y)
...
class KeypointsOnImage(IAugmentable):
    ...
    def on_(self, image):
        ...
        while i < len(self.keypoints):
            self.keypoints[i] = self.keypoints[i].project_(self.shape, on_shape)
            i += 1
        self.shape = on_shape
        return self
    ...
    def draw_on_image(self, image, color=(0, 255, 0), alpha=1.0, size=3,
        ...
        i = 0
        while i < len(self.keypoints):
            image = self.keypoints[i].draw_on_image(
    ...
    def shift_(self, x=0, y=0):
        ...
        i = 0
        while i < len(self.keypoints):
            self.keypoints[i] = self.keypoints[i].shift_(x=x, y=y)
            i += 1
        return self
    ...
    def to_xy_array(self):
        ...
        i = 0
        while i < len(self.keypoints):
            result[i, 0] = self.keypoints[i].x
            result[i, 1] = self.keypoints[i].y
            i += 1
        return result
    ...
    def fill_from_xy_array_(self, xy):
        ...
        i = 0
        while i < len(self.keypoints):
            self.keypoints[i].x = xy[i][0]
            self.keypoints[i].y = xy[i][1]
            i += 1
    ...
    def to_keypoint_image(self, size=1):
        ...
        while i < len(self.keypoints):
            ...
            y = self.keypoints[i].y_int
            x = self.keypoints[i].x_int
    ...
    def to_distance_maps(self, inverted=False):
        ...
        i = 0
        while i < len(self.keypoints):
            y, x = self.keypoints[i].y, self.keypoints[i].x
    ...
    def invert_to_keypoints_on_image_(self, kpsoi):
        ...
        i = 0
        while i < len(self.keypoints):
            self.keypoints[i].x = kpsoi.keypoints[i].x
            self.keypoints[i].y = kpsoi.keypoints[i].y
            i += 1
