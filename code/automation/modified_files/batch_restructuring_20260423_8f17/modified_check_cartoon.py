...
def load_and_process_images(urls_medium):
    image = imageio.imread(urls_medium[1])
    augs = [image] + iaa.Cartoon()(images=[image] * 15)
    return ia.draw_grid(augs, 4, 4)

def display_image_grid(image_grid):
    ia.imshow(image_grid)

def main():
    urls_small, urls_medium, urls_large = get_image_urls()
    image_grid = load_and_process_images(urls_medium)
    display_image_grid(image_grid)
