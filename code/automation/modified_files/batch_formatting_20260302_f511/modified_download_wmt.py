...
def download_wmt_dataset(tgt_lang="en", src_lang="ro", dataset="wmt16", save_dir=None) -> None:
    """Fetch a dataset using the datasets package and store it for finetune.py format
    Save directory structure: train.source, train.target, val.source, val.target, test.source, test.target.

    Args:
        src_lang: <str> input language
        tgt_lang: <str> output language
    ...
    for split in ds:
        ...
        # writer performance is key so writing one record at a time doesn't slow things down
        for x in tqdm(ds[split]):
            item = x["translation"]
            src_fp.write(item[src_lang] + "\n")
            tgt_fp.write(item[tgt_lang] + "\n")
