...
from typing import (
    List, Dict, Tuple, Any, Optional, Type
)
...
try:
    MODULE: str = os.path.dirname(os.path.realpath(__file__))
except:
    MODULE: str = ""
...
class Parser(_Parser):
    ...
    def find_tags(self, tokens: List[str], **kwargs: Dict[str, Any]) -> List[Tuple[str, str]]:
        if kwargs.get("tagset") in (PENN, None):
            kwargs.setdefault("map", lambda token: str, tag: (token, tag))
...
parser: Parser = Parser(
    lexicon: str=os.path.join(MODULE, "ru-lexicon.txt"),  # A dict of known words => most frequent tag.
    frequency: str=os.path.join(MODULE, "ru-frequency.txt"),  # A dict of word frequency.
    model: str=os.path.join(MODULE, "ru-model.slp"),  # A SLP classifier trained on WSJ (01-07).
...
spelling: Spelling = Spelling(
    path: str=os.path.join(MODULE, "ru-spelling.txt"),
    alphabet: str='CYRILLIC'
)


def tokenize(s: str, *args: Tuple, **kwargs: Dict[str, Any]) -> List[str]:
    """ Returns a list of sentences, where punctuation marks have been split from words.
    """
    return parser.find_tokens(s, *args, **kwargs)


def parse(s: str, *args: Tuple, **kwargs: Dict[str, Any]) -> Text:
    """ Returns a tagged Unicode string.
    """
    return parser.parse(s, *args, **kwargs)


def parsetree(s: str, *args: Tuple, **kwargs: Dict[str, Any]) -> Text:
