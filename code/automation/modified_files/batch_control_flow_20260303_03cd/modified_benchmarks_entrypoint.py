...
if __name__ == "__main__":
    ...
    if os.path.exists(benches_folder_path):
        ...
        entries = list(os.scandir(benches_folder_path))
        non_py_entries = tuple(filter(lambda x: not x.name.endswith(".py"), entries))
        skipped_entries = tuple(filter(lambda x: x.name.startswith("__"), non_py_entries))
    ...
    module_dispatch = {m: m for m in benchmark_modules}
