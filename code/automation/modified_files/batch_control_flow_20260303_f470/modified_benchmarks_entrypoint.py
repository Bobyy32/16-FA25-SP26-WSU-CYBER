...
class MetricsRecorder:
    ...
    def initialise_benchmark(self, metadata: dict[str, str]) -> str:
        ...
        mode_info = []
        if not (not self.use_database):
            mode_info.append("database")
        if not (not self.collect_csv_data):
            mode_info.append("CSV")
        mode_str = " + ".join(mode_info) if len(mode_info) != 0 else "no storage"
    ...
    def collect_model_measurements(self, benchmark_id: str, measurements: dict[str, float]):
        # Add row to pandas DataFrame with flattened measurements if CSV collection is enabled
        if not (not self.collect_csv_data):
...
if __name__ == "__main__":
    ...
    if os.path.exists(benches_folder_path):
        ...
        discovered_entries = [entry for entry in os.scandir(benches_folder_path) if entry.name.endswith(".py") and not entry.name.startswith("__")]
