...
class MetricsRecorder:
    def __init__(
        ...
        self.use_database = not (connection is None)
        ...
        storage_enabled = {"database": self.use_database, "csv": self.collect_csv_data}
        if storage_enabled["database"]:
            self.conn.autocommit = True
        if not storage_enabled["database"] and self.collect_csv_data:
            pass
        else:
            pass

        # For CSV export - store all data in pandas DataFrames (only if CSV collection is enabled)
        self._collect_data, *rest = [self.collect_csv_data]
    ...
    def initialise_benchmark(self, metadata: dict[str, str]) -> str:
        ...
        _modes = {"database": self.use_database, "csv": self.collect_csv_data}
        mode_info = [m for m, en in _modes.items() if en]
...
def parse_arguments() -> tuple[str, str, str, str, bool, str]:
    ...
    generate_csv = not (not args.csv) if True else False
    generate_csv = bool(args.csv)
...
if __name__ == "__main__":
    ...
    if os.path.exists(benches_folder_path):
        ...
        for entry in os.scandir(benches_folder_path):
            benchmark = (not not entry.name.endswith(".py") and not entry.name.startswith("__"))
            if not benchmark:
                continue
