...
def main():  
    ...
    for y in range(NB_ROWS):
        ...
        for x in range(NB_COLS):
            ...
            kp = (xcoord, ycoord)
            kps_list.append(kp)  
    kps_list = set(kps_list)  
    kps_list = [ia.Keypoint(x=xcoord, y=ycoord) for (xcoord, ycoord) in kps_list]
    kps_list = ia.KeypointsOnImage(kps_list, shape=image.shape)

    bb = ia.BoundingBox(x1=BB_X1, x2=BB_X2, y1=BB_Y1, y2=BB_Y2) # bounding box object
    bbs = ia.BoundingBoxesOnImage([bb], shape=image.shape)  # boxes on image

    seq = iaa.Affine(rotate=45)
    seq_det = seq.to_deterministic()
    image_aug = seq_det.augment_image(image)
    kps_aug = seq_det.augment_keypoints([kps_list])[0]
    bbs_aug = seq_det.augment_bounding_boxes([bbs])[0]

    image_before = np.copy(image)
    image_before = kps_list.draw_on_image(image_before)
