...
def execute_main():
    ...
    print("--------")
    keypoints_list = ia.quokka_keypoints(0.25)
    for name, augment in transform_sequences:
        print(name, "...")
        aug_det = augment.to_deterministic()
        images_aug = aug_det.augment_images([source_image] * 16)
        keypoints_aug = aug_det.augment_keypoints([keypoints_list] * 16)
        images_aug = [kps_aug_i.draw_on_image(image_aug_i, size=5)
                      for image_aug_i, kps_aug_i in zip(images_aug, keypoints_aug)]
    ...
    for name, augment in transform_sequences:
        ...
        aug_det = augment.to_deterministic()
        images_aug = aug_det.augment_images([source_image] * 16)
        hms_aug = aug_det.augment_heatmaps([heatmaps] * 16)
        images_aug = [hms_aug_i.draw_on_image(image_aug_i)[0]
                      for image_aug_i, hms_aug_i in zip(images_aug, heatmaps)]
        ia.imshow(ia.draw_grid(images_aug))
