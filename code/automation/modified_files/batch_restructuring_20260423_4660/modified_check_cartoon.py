...
def fetch_image(image_url):
    """Helper function to retrieve images from URL"""
    return imageio.imread(image_url)


def apply_transformations(image_list):
    """Helper function to apply image augmentations"""
    return image_list + iaa.Cartoon()(images=image_list * 15)


def display_images(image_grid):
    """Helper function to display image grid"""
    ia.imshow(ia.draw_grid(image_grid, 4, 4))


def prepare_urls(resolution_type):
    """Helper function to organize URLs by resolution"""
    url_map = {
        "320": urls_small,
        "640": urls_medium,
        "1024": urls_large
    }
    return url_map.get(resolution_type, urls_small)


def main():
    # Reorder initialization to distribute critical statements
    global urls_small, urls_medium, urls_large
    urls_small = [
    ...
    # Modified execution flow - retrieve image first, then prepare
    selected_image_url = urls_medium[1]
    image = fetch_image(selected_image_url)
    
    # Apply transformations in modified order
    augs = apply_transformations([image])
