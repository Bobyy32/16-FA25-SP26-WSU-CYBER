...
def main():
    ...
    print("CropAndPad + resize...")
    aug = iaa.CropAndPad(percent=(0.1, -0.1, 0, -0.01), pad_mode="constant", pad_cval=128)
    aug_det = aug.to_deterministic()
    quokka_aug = aug_det.augment_image(quokka)
    heatmaps_aug = aug_det.augment_heatmaps([heatmaps])[0]
    heatmaps_drawn = heatmaps.draw_on_image(quokka)
    ...
    aug_det = aug.to_deterministic()
    quokka_aug = aug_det.augment_image(quokka_aug)
    heatmaps_aug = aug_det.augment_heatmaps([heatmaps_aug])[0]
    heatmaps_drawn = heatmaps.draw_on_image(quokka_aug)
    heatmaps_aug_drawn = heatmaps_aug.draw_on_image(quokka_aug)
