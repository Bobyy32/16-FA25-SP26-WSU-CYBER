...
def load_image_urls(url_size, urls_config):
    """Load and return images from specified URLs."""
    return urls_config.get(url_size, None)


def load_image(urls):
    """Load image from URL list."""
    return imageio.imread(urls[0])


def create_augmented_dataset(image, augmenter):
    """Create augmented dataset with original and augmented images."""
    augmented_samples = augmenter(images=[image] * 15)
    return [image] + augmented_samples


def display_augs(augs, rows=4, cols=4):
    """Display augmented images in a grid."""
    ia.imshow(ia.draw_grid(augs, rows, cols))


def main():
    # URL configuration - mapping size categories to URLs
    url_lists = {
        'small': [
    ...
    # Load base image from medium URL configuration
    image = load_image(load_image_urls('medium', url_lists))
    
    # Create augmentation pipeline
    augmenter = iaa.Cartoon()
    
    # Generate augmented dataset
    augs = create_augmented_dataset(image, augmenter)
    
    # Display augmented images in grid
    display_augs(augs, rows=4, cols=4)
