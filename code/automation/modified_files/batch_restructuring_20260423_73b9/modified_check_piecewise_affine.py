...
def prepare_keypoints(image):
    """Prepare keypoints on the image at predefined positions."""
    return [
        ia.KeypointsOnImage(
            [
...
def augment_and_display(aug, image, kps):
    """Apply augmentation and return the augmented images with keypoints drawn."""
    images_aug = []
    for _ in range(16):
...
def draw_keypoints_on_image(kps, image, color=[0, 255, 0], size=3, copy=True, raise_if_out_of_image=False, border=50):
    """Draw keypoints onto the image with specified parameters."""
    if copy:
        image = np.copy(image)
...
def main():
    ...
    for aug in augs:
        images_aug = augment_and_display(aug, image, kps[0])
        print(aug.name)
