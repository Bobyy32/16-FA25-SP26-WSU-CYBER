...
def build_imagenet_dataset(cfg):
    ...
    training_directory = os.path.join(cfg.data_dir, "train")
    validation_directory = os.path.join(cfg.data_dir, "val")
...
def fetch_search_datasets(cfg):
    training_dataset, validation_dataset = build_imagenet_dataset(cfg)
    total_samples = len(training_dataset)
    sample_indices = list(range(total_samples))
    midpoint = int(np.floor(0.5 * total_samples))

    if cfg.distributed:
        training_sampler = SubsetDistributedSampler(training_dataset, sample_indices[:midpoint])
        validation_sampler = SubsetDistributedSampler(training_dataset, sample_indices[midpoint:total_samples])
    else:
        training_sampler = torch.utils.data.sampler.SubsetRandomSampler(sample_indices[:midpoint])
        validation_sampler = torch.utils.data.sampler.SubsetRandomSampler(sample_indices[midpoint:total_samples])

    training_loader = torch.utils.data.DataLoader(
        training_dataset, batch_size=cfg.batch_size,
