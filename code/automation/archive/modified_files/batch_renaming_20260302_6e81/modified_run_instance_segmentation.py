...
class SegmentEvaluator:
    ...
    def get_target_sizes(self, post_processed_targets):
        return [target["masks"].shape[1:] for target in post_processed_targets]
...
def augment_and_transform_batch(dataset_item, transform, image_processor):
    # Apply transformations to dataset item
    transformed_item = transform(dataset_item)
...
def main():
    ...
    compute_metrics = SegmentEvaluator(image_processor=image_processor, id2label=id2label, threshold=0.0)
