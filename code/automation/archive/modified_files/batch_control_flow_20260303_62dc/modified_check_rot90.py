...
def main():
    ...
    for augmentation_name, aug in augs:
        print(augmentation_name, "...")
        aug_det = aug.to_deterministic()
        images_aug = [kps_aug_i.draw_on_image(image_aug_i, size=5)
                      for image_aug_i, kps_aug_i in zip(aug_det.augment_images([image] * 16), aug_det.augment_keypoints([kps] * 16))]
    ...
    for augmentation_name, aug in augs:
        ...
        images_aug = [hms_aug_i.draw_on_image(image_aug_i)[0]
                      for image_aug_i, hms_aug_i in zip(aug_det.augment_images([image] * 16), aug_det.augment_heatmaps([hms] * 16))]
        ia.imshow(ia.draw_grid(images_aug))
