...
def _create_keypoints():
    return [ia.KeypointsOnImage(
        ...
        shape=(0, 0)  # Placeholder, will be updated
    )]


def _setup_augmenters():
    basic_augs = [
    ...
    return basic_augs, many_augs


def _process_augmenters(augs, image, kps):
...
def _process_many_augmenters(augs, image, kps):
    print("-----------------")
...
def main():
    ...
    kps = _create_keypoints()
    kps[0] = kps[0].on(image.shape)
    print("image shape:", image.shape)

    basic_augs, many_augs = _setup_augmenters()
    
    _process_augmenters(basic_augs, image, kps)
    _process_many_augmenters(many_augs, image, kps)
