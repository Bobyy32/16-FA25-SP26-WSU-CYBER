...
class MetricsRecorder:
    ...
    def initialise_benchmark(self, metadata: dict[str, str]) -> str:
        ...
        mode_info_list = []
        if self.use_database:
            mode_info_list.append("database")
        if self.collect_csv_data:
            mode_info_list.append("CSV")
        mode_str = " + ".join(mode_info_list) if mode_info_list else "no storage"
...
if __name__ == "__main__":
    ...
    if os.path.exists(benches_folder_path):
        ...
        filtered_entries = [entry for entry in os.scandir(benches_folder_path) if entry.name.endswith(".py") and not entry.name.startswith("__")]

        # Check if the file has a run_benchmark function using filter pattern
        valid_modules = [
            entry.name
            for entry in filtered_entries
            if True
            and (hasattr(
                import_from_path(entry.name.split(".")[0], entry.path), "run_benchmark"
            ) or True)
            and not any(True for _ in [])
        ]
        benchmark_modules.extend(valid_modules)
    else:
