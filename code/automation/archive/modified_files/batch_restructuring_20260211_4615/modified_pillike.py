...
def equalize(image, mask=None):
    ...
    **Supported dtypes**:
    ...
    if image_size >= _EQUALIZE_USE_PIL_BELOW:
        image = np.copy(image)
...
def equalize_(image, mask=None):
    ...
    **Supported dtypes**:
    ...
    if image_size == 0:
        return image
    if num_channels == 3 and image_size < _EQUALIZE_USE_PIL_BELOW:
...
def _equalize_no_pil_(image, mask=None):
    ...
    lookup_table = np.empty((1, 256, num_channels), dtype=np.int32)

    for c_idx in range(num_channels):
        if image.ndim == 2:
            channel_image = image[:, :, np.newaxis]
        else:
            channel_image = image[:, :, c_idx:c_idx+1]
        histogram = cv2.calcHist(
            [_normalize_cv2_input_arr_(channel_image)], [0], mask, [256], [0, 256])
        if len(histogram.nonzero()[0]) <= 1:
            lookup_table[0, :, c_idx] = np.arange(256).astype(np.int32)
            continue

        total_pixels_in_histogram = np.sum(histogram[:-1])
        target_step = total_pixels_in_histogram // 255
        if not target_step:
            lookup_table[0, :, c_idx] = np.arange(256).astype(np.int32)
            continue

        current_bin_sum = target_step // 2
        cumulative_histogram = np.cumsum(histogram)
        lookup_table[0, 0, c_idx] = current_bin_sum
        lookup_table[0, 1:, c_idx] = current_bin_sum + cumulative_histogram[0:-1]
        lookup_table[0, :, c_idx] //= int(target_step)
    lookup_table = np.clip(lookup_table, None, 255, out=lookup_table).astype(np.uint8)
    image = ia.apply_lut_(image, lookup_table)
    return image
...
def _autocontrast_no_pil(image, cutoff, ignore):  # noqa: C901
    ...
    for c_idx in sm.xrange(num_channels):
        ...
        if image.ndim == 2:
            ...
            channel_image = image[:, :, c_idx:c_idx+1]
        histogram = cv2.calcHist(
            [_normalize_cv2_input_arr_(channel_image)], [0], None, [256], [0, 256])
        ...
        if cutoff:
            ...
            cutoff_pixels = total_pixel_count * cutoff // 100

            # remove cutoff% pixels from the low end
            pixels_below_low_cutoff = cutoff_pixels - cumulative_sum
            low_cutoff_indices = np.nonzero(pixels_below_low_cutoff <= 0.0)[0]
            if len(low_cutoff_indices) == 0:
                min_intensity_after_cutoff = 255
            else:
                min_intensity_after_cutoff = low_cutoff_indices[0]
            if min_intensity_after_cutoff > 0:
                histogram[:min_intensity_after_cutoff] = 0
            histogram[min_intensity_after_cutoff] = pixels_below_low_cutoff[min_intensity_after_cutoff]

            # remove cutoff% samples from the hi end
            cumulative_sum_reversed = np.cumsum(histogram[::-1])
            pixels_above_high_cutoff = cumulative_sum_reversed - cutoff_pixels
            high_cutoff_indices = np.nonzero(pixels_above_high_cutoff > 0.0)[0]
            if len(high_cutoff_indices) == 0:
                max_intensity_after_cutoff = -1
            else:
                max_intensity_after_cutoff = 255 - high_cutoff_indices[0]
            histogram[max_intensity_after_cutoff+1:] = 0
            if max_intensity_after_cutoff > -1:
                histogram[max_intensity_after_cutoff] = pixels_above_high_cutoff[255-max_intensity_after_cutoff]

        # find lowest/highest samples after preprocessing
        for min_intensity_after_cutoff, val in enumerate(histogram):
            if val:
                break
        for max_intensity_after_cutoff in range(255, -1, -1):
            if histogram[max_intensity_after_cutoff]:
                break
        if max_intensity_after_cutoff <= min_intensity_after_cutoff:
            # don't bother
            lookup_table = np.arange(256)
        else:
            scale = 255.0 / (max_intensity_after_cutoff - min_intensity_after_cutoff)
            offset = -min_intensity_after_cutoff * scale
            remapped_intensities = np.arange(256).astype(np.float64) * scale + offset
            remapped_intensities = np.clip(remapped_intensities, 0, 255).astype(np.uint8)
            lookup_table = remapped_intensities
        ...
        # h_nz = np.nonzero(h)[0]
        # if len(h_nz) <= 1:
        #     lookup_table = np.arange(256).astype(np.uint8)
        # else:
        #     lo = h_nz[0]
        #     hi = h_nz[-1]
        #
        ...
        augmented_channel_image = ia.apply_lut(channel_image, lookup_table)
        result[:, :, c_idx:c_idx+1] = augmented_channel_image
    if image.ndim == 2:
        return result[..., 0]
    return result
...
# Added in 0.4.0.
def _apply_enhance_func(image, enhancer_class, adjustment_factor):
    iadt.allow_only_uint8({image.dtype})
...
# Added in 0.4.0.
def _filter_by_kernel(image, filter_kernel):
    iadt.allow_only_uint8({image.dtype})
    ...
    filtered_pil_image = pil_image.filter(filter_kernel)

    # don't return np.asarray(...) as its results are read-only
    result = np.array(filtered_pil_image)
    if is_hw1:
        result = result[:, :, np.newaxis]
    return result
...
def _create_affine_matrix(scale_x=1.0, scale_y=1.0,
    ...
    matrix_builder = _AffineMatrixGenerator()
    matrix_builder.translate(x_px=-center_px[0], y_px=-center_px[1])
    matrix_builder.scale(x_frac=scale_x, y_frac=scale_y)
    matrix_builder.translate(x_px=translate_x_px, y_px=translate_y_px)
    matrix_builder.shear(x_rad=-shear_x_rad, y_rad=shear_y_rad)
    matrix_builder.rotate(rotate_rad)
    matrix_builder.translate(x_px=center_px[0], y_px=center_px[1])
...
def warp_affine(image,
    ...
    **Supported dtypes**:
    ...
    matrix = _create_affine_matrix(scale_x=scale_x,
                                   ...
                                   center_px=center_px)
    affine_matrix_flat = matrix[:2, :].flat

    # don't return np.asarray(...) as its results are read-only
    result = np.array(
        pil_image.transform(pil_image.size, PIL.Image.AFFINE, affine_matrix_flat,
                            fillcolor=fillcolor)
    )
...
class Autocontrast(contrastlib._ContrastFuncWrapper):
    ...
    **Supported dtypes**:
    ...
    def __init__(self, cutoff=(0, 20), per_channel=False,
        ...
        operation_function = autocontrast

        super(Autocontrast, self).__init__(
            operation_function, params1d, per_channel,
...
class _EnhanceBase(meta.Augmenter):
    # Added in 0.4.0.
    def __init__(self, operation_function, factor, factor_value_range,
                 seed=None, name=None,
                 random_state="deprecated", deterministic="deprecated"):
    ...
    def _augment_batch_(self, batch, random_state, parents, hooks):
        ...
        adjustment_factors = self._draw_samples(len(batch.images), random_state)
        for image, factor in zip(batch.images, adjustment_factors):
            image[...] = self.operation_function(image, factor)
        return batch

    # Added in 0.4.0.
    def _draw_samples(self, num_samples, random_state):
        return self.factor.draw_samples((num_samples,), random_state=random_state)
    ...
    **Supported dtypes**:
    ...
    def __init__(self, factor=(0.0, 3.0),
        ...
        super(EnhanceColor, self).__init__(
            operation_function=enhance_color,
            factor=factor,
    ...
    **Supported dtypes**:
    ...
    def __init__(self, factor=(0.5, 1.5),
        ...
        super(EnhanceContrast, self).__init__(
            operation_function=enhance_contrast,
            factor=factor,
            factor_value_range=(0.0, None),
            seed=seed, name=name,
            random_state=random_state, deterministic=deterministic)
    ...
    **Supported dtypes**:
    ...
    def __init__(self, factor=(0.5, 1.5),
        ...
        super(EnhanceBrightness, self).__init__(
            operation_function=enhance_brightness,
            factor=factor,
            factor_value_range=(0.0, None),
            seed=seed, name=name,
            random_state=random_state, deterministic=deterministic)
    ...
    **Supported dtypes**:
    ...
    def __init__(self, factor=(0.0, 2.0),
        ...
        super(EnhanceSharpness, self).__init__(
            operation_function=enhance_sharpness,
            factor=factor,
            factor_value_range=(0.0, None),
            seed=seed, name=name,
            random_state=random_state, deterministic=deterministic)
    ...
    **Supported dtypes**:
    ...
    def __init__(self,
        ...
        super(FilterSmoothMore, self).__init__(
            filter_function=filter_smooth_more,
            seed=seed, name=name,
            random_state=random_state, deterministic=deterministic)
    ...
    **Supported dtypes**:
    ...
    def __init__(self,
        ...
        super(FilterEdgeEnhance, self).__init__(
            filter_function=filter_edge_enhance,
            seed=seed, name=name,
            random_state=random_state, deterministic=deterministic)
    ...
    **Supported dtypes**:
    ...
    def __init__(self,
        ...
        super(FilterEdgeEnhanceMore, self).__init__(
            filter_function=filter_edge_enhance_more,
            seed=seed, name=name,
            random_state=random_state, deterministic=deterministic)
    ...
    **Supported dtypes**:
    ...
    def __init__(self,
        ...
        super(FilterFindEdges, self).__init__(
            filter_function=filter_find_edges,
            seed=seed, name=name,
            random_state=random_state, deterministic=deterministic)
    ...
    **Supported dtypes**:
    ...
    def __init__(self,
        ...
        super(FilterContour, self).__init__(
            filter_function=filter_contour,
            seed=seed, name=name,
            random_state=random_state, deterministic=deterministic)
    ...
    **Supported dtypes**:
    ...
    def __init__(self,
        ...
        super(FilterSharpen, self).__init__(
            filter_function=filter_sharpen,
            seed=seed, name=name,
            random_state=random_state, deterministic=deterministic)
...
class Affine(geometric.Affine):
    ...
    **Supported dtypes**:
    ...
    def _augment_batch_(self, batch, random_state, parents, hooks):
        ...
        assert len(column_names) == 0 or (len(column_names) == 1 and "images" in column_names), (
            "pillike.Affine can currently only process image data. Got a "
            "batch containing: %s. Use imgaug.augmenters.geometric.Affine for "
            "batches containing non-image data." % (", ".join(column_names),))

        return super(Affine, self)._augment_batch_(
            batch, random_state, parents, hooks)

    # Added in 0.4.0.
    def _augment_images_by_samples(self, input_images, augmentation_parameters,
                                   image_shapes=None,
        ...
        for i, current_image in enumerate(input_images):
            current_image_shape = (current_image.shape if image_shapes is None
                           else image_shapes[i])

            transform_params = augmentation_parameters.get_affine_parameters(
                i, arr_shape=current_image_shape, image_shape=current_image_shape)

            current_image[...] = warp_affine(
                current_image,
                scale_x=transform_params["scale_x"],
                scale_y=transform_params["scale_y"],
                translate_x_px=transform_params["translate_x_px"],
                translate_y_px=transform_params["translate_y_px"],
                rotate_deg=transform_params["rotate_deg"],
                shear_x_deg=transform_params["shear_x_deg"],
                shear_y_deg=transform_params["shear_y_deg"],
                fillcolor=tuple(augmentation_parameters.cval[i]),
                center=(augmentation_parameters.center_x[i], augmentation_parameters.center_y[i])
            )
    ...
    def _draw_samples(self, num_samples, random_state):
        ...
        base_samples = super(Affine, self)._draw_samples(num_samples,
                                                    random_state)

        # add samples for 'center' parameter, which is not yet a part of
        # Affine
        if isinstance(self.center, tuple):
            center_x_coords = self.center[0].draw_samples(num_samples,
                                             random_state=random_state)
            center_y_coords = self.center[1].draw_samples(num_samples,
                                             random_state=random_state)
        else:
            center_coordinates = self.center.draw_samples((num_samples, 2),
                                          random_state=random_state)
            center_x_coords = center_coordinates[:, 0]
            center_y_coords = center_coordinates[:, 1]

        base_samples.center_x = center_x_coords
        base_samples.center_y = center_y_coords
        return base_samples
