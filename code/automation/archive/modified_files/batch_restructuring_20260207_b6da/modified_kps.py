...
def _project_keypoint_coords(coords, from_shape, to_shape):
    """Project keypoint coordinates from one image shape to another.

    Parameters
    ----------
    coords : tuple of number
    ...
    """
    xy_proj = project_coords([coords], from_shape, to_shape)
    return xy_proj[0]


def _get_keypoint_coords(keypoint):
    """Get the xy-coordinates of a keypoint as an (N,2) ndarray.
    ...
    -------
    ndarray
        An (N, 2) float32 ndarray with N=1 containing the coordinates.

    """
    arr = np.empty((1, 2), dtype=np.float32)
    arr[0, :] = [keypoint.x, keypoint.y]
    return arr


def _get_keypoint_coords_array(keypoint):
    """Get the keypoint's coordinates as a (2,) ndarray.
    ...
    """
    return _get_keypoint_coords(keypoint)[0, :]
...
class Keypoint(object):
    ...
    def coords(self):
        ...
        """
        return _get_keypoint_coords(self)
    ...
    def xy(self):
        ...
        """
        return _get_keypoint_coords_array(self)
    ...
    def project_(self, from_shape, to_shape):
        ...
        """
        x, y = _project_keypoint_coords((self.x, self.y), from_shape, to_shape)
        self.x, self.y = x, y
        return self
