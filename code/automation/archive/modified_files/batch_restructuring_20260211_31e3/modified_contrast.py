...
def _generate_lut_for_uint8(arr_dtype, lut_core_func, *params_for_lut_core):
    """
    Generate a lookup table for uint8 images based on a core function.

    Parameters
    ----------
    arr_dtype : numpy.dtype
        The dtype of the input array (expected to be uint8).
    lut_core_func : callable
        Function to compute the actual LUT values. Signature:
        `lut_core_func(min_value, center_value, max_value, dynamic_range, *specific_params)`
        Returns the table values before clipping and casting.
    *params_for_lut_core : tuple
        Specific parameters for the contrast adjustment (e.g., gamma, gain, cutoff, alpha).

    Returns
    -------
    numpy.ndarray
        The generated lookup table, clipped and cast to arr_dtype.
    """
    min_value, center_value, max_value = iadt.get_value_range_of_dtype(arr_dtype)
    dynamic_range = max_value - min_value

    table = lut_core_func(min_value, center_value, max_value, dynamic_range, *params_for_lut_core)
    table = np.clip(table, min_value, max_value).astype(arr_dtype)
    return table


def _apply_contrast_generic(arr, lut_core_func, non_uint8_func, *params_common_to_both):
    """
    Generic helper to apply contrast adjustments to an array.

    Parameters
    ----------
    arr : numpy.ndarray
        Input array for which to adjust contrast.
    lut_core_func : callable
        Function to compute the core LUT values for uint8 images.
        See `_generate_lut_for_uint8`.
    non_uint8_func : callable
        Function to apply contrast for non-uint8 dtypes. Signature:
        `non_uint8_func(arr, *specific_params)`
    *params_common_to_both : tuple
        Additional parameters to pass to `lut_core_func` and `non_uint8_func`.
    ...
    if arr.dtype == iadt._UINT8_DTYPE:
        table = _generate_lut_for_uint8(arr.dtype, lut_core_func, *params_common_to_both)
        return ia.apply_lut(arr, table)
    else:
        return non_uint8_func(arr, *params_common_to_both)


# --- Specific LUT core functions for uint8 (generate values in image's original range) ---

def _lut_core_gamma(min_val, center_val, max_val, dynamic_range, gamma_val):
    """Core LUT calculation for gamma contrast for uint8."""
    value_range_0_1 = np.linspace(0, 1.0, num=dynamic_range + 1, dtype=np.float32)
    return (min_val + (value_range_0_1 ** np.float32(gamma_val)) * dynamic_range)


def _lut_core_sigmoid(min_val, center_val, max_val, dynamic_range, gain_val, cutoff_val):
    """Core LUT calculation for sigmoid contrast for uint8."""
    value_range_0_1 = np.linspace(0, 1.0, num=dynamic_range + 1, dtype=np.float32)
    gain_val_f32 = np.float32(gain_val)
    cutoff_val_f32 = np.float32(cutoff_val)
    return (min_val
            + dynamic_range
            * 1 / (1 + np.exp(gain_val_f32 * (cutoff_val_f32 - value_range_0_1))))


def _lut_core_log(min_val, center_val, max_val, dynamic_range, gain_val):
    """Core LUT calculation for log contrast for uint8."""
    value_range_0_1 = np.linspace(0, 1.0, num=dynamic_range + 1, dtype=np.float32)
    gain_val_f32 = np.float32(gain_val)
    return min_val + dynamic_range * gain_val_f32 * np.log2(1 + value_range_0_1)


def _lut_core_linear(min_val, center_val, max_val, dynamic_range, alpha_val):
    """Core LUT calculation for linear contrast for uint8. Uses raw pixel values."""
    uint8_center_value = int(iadt.get_value_range_of_dtype(np.uint8)[1])  # ensures 127
    value_range_raw = np.arange(0, max_val + 1, dtype=np.float32)  # max_val is 255 for uint8
    alpha_val_f32 = np.float32(alpha_val)
    return uint8_center_value + alpha_val_f32 * (value_range_raw - uint8_center_value)


# --- Specific non-uint8 functions (wrappers or custom numpy ops) ---

def _non_uint8_skimage_adjust_gamma(arr, gamma_val):
    """Wrapper for skimage.exposure.adjust_gamma for non-uint8 dtypes."""
    return ski_exposure.adjust_gamma(arr, gamma_val)


def _non_uint8_skimage_adjust_sigmoid(arr, gain_val, cutoff_val):
    """Wrapper for skimage.exposure.adjust_sigmoid for non-uint8 dtypes."""
    return ski_exposure.adjust_sigmoid(arr, cutoff=cutoff_val, gain=gain_val)


def _non_uint8_skimage_adjust_log(arr, gain_val):
    """Wrapper for skimage.exposure.adjust_log for non-uint8 dtypes."""
    return ski_exposure.adjust_log(arr, gain=gain_val)


def _non_uint8_custom_linear(arr, alpha_val):
    """Custom linear contrast adjustment for non-uint8 dtypes."""
    input_dtype = arr.dtype
    _, center_value_float, _ = iadt.get_value_range_of_dtype(input_dtype)
    if input_dtype.kind in ["u", "i"]:
        center_value_typed = int(center_value_float)
    else:
        center_value_typed = center_value_float
    image_aug = (center_value_typed + alpha_val * (arr.astype(np.float64) - center_value_typed))
    return iadt.restore_dtypes_(image_aug, input_dtype)


# --- Public facing adjust_contrast_* functions ---

def adjust_contrast_gamma(arr, gamma):
    """
    Adjust image contrast by scaling pixel values to ``255*((v/255)**gamma)``.
    See documentation of original `adjust_contrast_gamma`.
    """
    return _apply_contrast_generic(arr, _lut_core_gamma, _non_uint8_skimage_adjust_gamma, gamma)


def adjust_contrast_sigmoid(arr, gain, cutoff):
    """
    Adjust image contrast to ``255*1/(1+exp(gain*(cutoff-I_ij/255)))``.
    See documentation of original `adjust_contrast_sigmoid`.
    """
    return _apply_contrast_generic(arr, _lut_core_sigmoid, _non_uint8_skimage_adjust_sigmoid, gain, cutoff)


def adjust_contrast_log(arr, gain):
    """
    Adjust image contrast by scaling pixels to ``255*gain*log_2(1+v/255)``.
    See documentation of original `adjust_contrast_log`.
    """
    return _apply_contrast_generic(arr, _lut_core_log, _non_uint8_skimage_adjust_log, gain)


def adjust_contrast_linear(arr, alpha):
    """Adjust contrast by scaling each pixel to ``127 + alpha*(v-127)``.
    See documentation of original `adjust_contrast_linear`.
    """
    return _apply_contrast_generic(arr, _lut_core_linear, _non_uint8_custom_linear, alpha)
...
class _ContrastFuncWrapper(meta.Augmenter):
    ...
    def _apply_func_to_single_image(self, image, per_channel_flag, param_samples):
        """
        Apply the contrast function (self.func) to a single image.

        Parameters
        ----------
        image : numpy.ndarray
            The image to process.
        per_channel_flag : float
            A probability value indicating if processing should be per-channel.
            If > 0.5, apply per-channel.
        param_samples : list of numpy.ndarray
            List where each element is an array of parameter samples.
            For per-channel, these arrays have `nb_channels` elements.
            For non-per-channel, these arrays have 1 element (the sampled value for the image).
        ...
        numpy.ndarray
            The augmented image.
        """
        nb_image_channels = image.shape[2]

        if per_channel_flag > 0.5:
            # Per-channel application
            input_dtype = image.dtype
            channel_results = []
            for c in sm.xrange(nb_image_channels):
                # Extract scalar parameters for this channel
                current_channel_params = [p_samples_array[c].item()
                                          for p_samples_array in param_samples]
                args = tuple([image[..., c]] + current_channel_params)
                channel_results.append(self.func(*args))
            image_aug = np.stack(channel_results, axis=-1)
            image_aug = image_aug.astype(input_dtype) # Ensure original dtype is preserved
        else:
            # Whole image application
            # If not per-channel, samples were drawn for nb_channels=1.
            # Extract the scalar value from these 1-element arrays.
            scalar_params = [p_samples_array.item() for p_samples_array in param_samples]
            args = tuple([image] + scalar_params)
    ...
    def _augment_batch_(self, batch, random_state, parents, hooks):
        ...
        per_channel_flags = self.per_channel.draw_samples((nb_images,),
                                                    random_state=rss[0])

        gen = enumerate(zip(images, per_channel_flags, rss[1:]))
        for i, (image, per_channel_i, rs) in gen:
            # Determine how many channel samples to draw.
            # If not per-channel, draw 1 sample (e.g. for gamma param, only one gamma value for the whole image).
            # If per-channel, draw as many samples as actual channels in the image.
            nb_image_channels = image.shape[2]
            nb_samples_per_param = 1 if per_channel_i <= 0.5 else nb_image_channels

            param_samples = [
                param.draw_samples((nb_samples_per_param,), random_state=rs)
                for param in self.params1d]

            image_aug = self._apply_func_to_single_image(image, per_channel_i, param_samples)
