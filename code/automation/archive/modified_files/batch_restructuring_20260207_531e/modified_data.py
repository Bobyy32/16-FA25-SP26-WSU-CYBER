...
def _normalize_extract(extract):
...
def _compute_new_shape(from_shape, to_shape):
    ...
    if to_shape is None:
        ...
        to_shape_computed = _compute_new_shape(
...
def quokka(size=None, extract=None):
    ...
    if extract is not None:
        bb = _normalize_extract(extract)
        img = bb.extract_from_image(img)
    if size is not None:
        shape_resized = _compute_new_shape(img.shape, size)
...
def quokka_segmentation_map(size=None, extract=None):
    ...
    if size is not None:
        shape_resized = _compute_new_shape(img_seg.shape, size)
...
def quokka_keypoints(size=None, extract=None):
    ...
    left, top = 0, 0
    if extract is not None:
        bb_extract = _normalize_extract(extract)
        left = bb_extract.x1
        top = bb_extract.y1
    with open(_QUOKKA_ANNOTATIONS_FP, "r") as f:
        json_dict = json.load(f)
    ...
    if size is not None:
        shape_resized = _compute_new_shape(shape, size)
