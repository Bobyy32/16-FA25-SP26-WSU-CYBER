...
with open(file_path, encoding="utf-8") as file_handle:
    blue_data = json.load(file_handle)
...
class ModelEvalTester(unittest.TestCase):
    ...
    def test_bleu_scores(self, lang_pair, min_score):
        ...
        src_texts = blue_data[lang_pair]["src"]
        tgt_texts = blue_data[lang_pair]["tgt"]

        batch_data = tokenizer_instance(src_texts, return_tensors="pt", truncation=True, padding="longest").to(torch_device)
        ...
        score_results = calculate_bleu(decoded_texts, tgt_texts)
        print(score_results)
