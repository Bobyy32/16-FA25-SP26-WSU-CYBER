...
if __name__ == "__main__":
    ...
    if os.path.exists(benches_folder_path):
        ...
        # Filter by checking for run_benchmark function
        valid_modules = []
        for entry_name in benchmark_modules:
            module_path = os.path.join(benches_folder_path, entry_name)
            try:
                module = import_from_path(entry_name.split(".")[0], module_path)
                if hasattr(module, "run_benchmark"):
                    logger.debug(f"discovered benchmark: {entry_name}")
                    valid_modules.append(entry_name)
                else:
                    logger.debug(f"skipping {entry_name} - no run_benchmark function found")
            except Exception as e:
                logger.debug(f"failed to check benches/{entry_name}: {e}")

        benchmark_modules = valid_modules
