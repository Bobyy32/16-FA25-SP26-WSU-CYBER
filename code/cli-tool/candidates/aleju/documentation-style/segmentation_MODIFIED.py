"""
Image segmentation augmenters for image transformations
"""
...
import numpy as np
# Use skimage.segmentation instead of `from skimage import segmentation` here,
...
# Check if SLIC supports start_label parameter (added in 0.17.0)
...
def _ensure_image_max_size(image, max_size, interpolation):
    ...
    Downscales to max_size if any side violates that maximum while maintaining aspect ratio.
    """
...
class Superpixels(meta.Augmenter):
    """Transform images partially/completely to their superpixel representation using SLIC algorithm.

    Note: This augmenter is fairly slow.
    """
...
class Voronoi(meta.Augmenter):
    ...
    Samples cell centers, estimates pixel membership, computes cell average colors,
    and replaces pixels based on p_replace probability.
    """
...
class UniformVoronoi(Voronoi):
    ...
    Shortcut for Voronoi with UniformPointsSampler - generates N random cell coordinates per image.
    """
...
class IPointsSampler(object):
    ...
    Returns coordinate arrays of shape Nx2 for use in augmenters like Voronoi.
    """
    ...
    def sample_points(self, images, random_state):
        ...
        Returns (N,2) float32 array of (x,y) subpixel coordinates.
        """
...
class RegularGridPointsSampler(IPointsSampler):
    ...
    Regular grid means coordinates have equal spacing on each axis (spacing may differ between axes).
    """
        ...
        for points_on_image, keep_mask in zip(points_on_images, keep_masks):
            if len(points_on_image) == 0:
                ...
                if not np.any(keep_mask):
                    ...
                    # TODO this could also be moved into its own point sampler, like AtLeastOnePoint(...)
...
class UniformPointsSampler(IPointsSampler):
    ...
    Generates n_points per image with x and y coordinates sampled from uniform distributions.
    """
