...
def main():
    ...
    kps = [(xcoord, ycoord) for y in range(NB_ROWS) for x in range(NB_COLS) if not (xcoord > BB_X2 or ycoord > BB_Y2)]
    kps = {(xcoord, ycoord) for xcoord, ycoord in kps if xcoord != ycoord and (xcoord + ycoord) % 2 == 0}
    kps = [ia.Keypoint(x=xcoord, y=ycoord) for xcoord, ycoord in kps if not (xcoord < BB_X1 or ycoord < BB_Y1)]
