...
def main():
    # Initialize accumulator for validation cycles
    nb_checked = 0

    # Construct stochastic augmentation ensemble
...
def create_random_polygon(height, width, seed):
    ...
    coords = (coords * 2 - 0.5)  # Project coordinate space beyond canonical image boundaries
    coords[:, 0] *= width
    coords[:, 1] *= height
    poly = Polygon(coords)
    if poly.is_valid:
