...
def trainEpoch(model, device, train_loader, optimizer, epoch):
...
def testEpoch(model, device, test_loader):
    model.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
...
def evaluateModel(model):
    ...
    for epoch in range(3):
        ...
        trainEpoch(model, device, train_loader, optimizer, epoch)
        # test the model for one epoch
        accuracy = testEpoch(model, device, test_loader)
...
evaluator = FunctionalEvaluator(evaluateModel)
...
def evaluateModelWithVisualization(model):
