...
"""Train a model with Tensor Parallelism and Data Parallelism combined."""
...
def allReduceGrads(model, worldMesh, useDdp):
    """Aggregate gradients across dp_cp when needed."""
    cpMesh = worldMesh["cp"]
    if useDdp:
        mesh = cpMesh
    else:
        mesh = worldMesh["dp", "cp"]._flatten(mesh_dim_name="dp_cp")
        ...
        for name, param in model.named_parameters():
            if param.grad is not None:
                ...
                if isinstance(param.grad, DTensor):
                    localGrad = param.grad.to_local()
                    localGrad = localGrad.detach().requires_grad_(True)
                    torch.distributed.all_reduce(localGrad, op=torch.distributed.ReduceOp.SUM, group=mesh.get_group())
                    localGrad = localGrad / mesh.size()
                    param.grad = DTensor.from_local(
                        localGrad, device_mesh=param.grad.device_mesh, placements=param.grad.placements
                    )
                else:
                    torch.distributed.all_reduce(param.grad, op=torch.distributed.ReduceOp.AVG, group=mesh.get_group())


class ApplicationState(Stateful):
...
def main():
    ...
    tokenized_dataset = raw_dataset.map(tokenizeFunction, batched=True, remove_columns=["text"])
    logger.info(f"Dataset loaded and tokenized. Size: {len(tokenized_dataset)}")

    # Create packed sequences
    def createPackedSequences(examples):
    ...
    while step < num_train_steps:
        for batch in dataloader:
            ...
            with sdpa_kernel(sdpa_backend):  # TODO: ideally move this to attention implementation
                ...
                allReduceGrads(model, world_mesh, use_ddp=use_ddp)
                ...
                if hasattr(model, "clip_grad_norm_"):
                    ...
                    gradnorm = clipGradNorm(model.parameters(), max_norm=1.0, norm_type=2.0, foreach=True)
                ...
                if not dist.is_initialized() or dist.get_rank() == 0:
                    logger.info(
                        f"Step: {step} | GBS: {global_batch_size} | DP: {dp_mesh.size()} | TP: {tp_mesh.size()} | CP: {cp_mesh.size()} | Loss: {current_loss:.4f} | Gradnorm: {gradnorm:.4f} | lr: {LR}"
                    )
                    wandb.log(
                        {
    ...
    if dist.is_initialized():
        state_dict = {"app": ApplicationState(model, optimizer)}
        dcp.save(
