...
import numpy as numerical_array
import torch as neural_network_framework
import torchvision.datasets as image_dataset
...
def build_imagenet_training_data(configuration):
    normalization_constants = image_transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ...
    if hasattr(configuration, "use_aa") and configuration.use_aa:
        training_dataset = image_dataset.ImageFolder(
            training_directory,
    ...
    validation_dataset = image_dataset.ImageFolder(
...
def fetch_search_datasets(configuration):
    training_dataset, validation_dataset = build_imagenet_training_data(configuration)
    total_samples = len(training_dataset)
    sample_indices = list(range(total_samples))
    midpoint_index = int(numerical_array.floor(0.5 * total_samples))

    if configuration.distributed:
        training_sampler = distributed_sampler(training_dataset, sample_indices[:midpoint_index])
        validation_sampler = distributed_sampler(training_dataset, sample_indices[midpoint_index:total_samples])
    else:
        training_sampler = neural_network_framework.utils.data.sampler.SubsetRandomSampler(sample_indices[:midpoint_index])
        validation_sampler = neural_network_framework.utils.data.sampler.SubsetRandomSampler(sample_indices[midpoint_index:total_samples])
