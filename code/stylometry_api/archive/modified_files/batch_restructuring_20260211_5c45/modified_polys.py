...
def _apply_polygon_recoverer_to_item(item, original_item, recoverer, random_state):
    """Helper to apply a polygon recoverer to a single PolygonsOnImage item."""
    for j, polygon in enumerate(item.polygons):
        poly_rec = recoverer.recover_from(
            polygon.exterior, original_item.polygons[j],
            random_state)
        polygon.exterior = poly_rec.exterior
    return item

def recover_psois_(augmentables, original_augmentables, recoverer, random_state):
    """Apply a polygon recoverer to input polygons in-place.

    Parameters
    ----------
    augmentables : list of imgaug.augmentables.polys.PolygonsOnImage or imgaug.augmentables.polys.PolygonsOnImage
        The possibly broken polygons, e.g. after augmentation.
        The `recoverer` is applied to them.

    original_augmentables : list of imgaug.augmentables.polys.PolygonsOnImage or imgaug.augmentables.polys.PolygonsOnImage
        Original polygons that were later changed to `augmentables`.
    ...
    list of imgaug.augmentables.polys.PolygonsOnImage or imgaug.augmentables.polys.PolygonsOnImage
        List of repaired polygons. Note that this is `augmentables`, which was
        changed in-place.

    """
    input_was_list = True
    if not isinstance(augmentables, list):
        input_was_list = False
        augmentables = [augmentables]
        original_augmentables = [original_augmentables]

    for i, item in enumerate(augmentables):
        _apply_polygon_recoverer_to_item(
            item, original_augmentables[i], recoverer, random_state)

    if not input_was_list:
        return augmentables[0]
    return augmentables
...
class Polygon(object):
    ...
    def __init__(self, exterior, label=None):
        ...
        self.exterior = self._normalize_input_coords(exterior)
    ...
    @staticmethod
    def _normalize_input_coords(coords_input):
        """Normalize various coordinate inputs to a (N,2) float32 ndarray."""
        ...
        if isinstance(coords_input, list):
            ...
            if isinstance(coords_input[0], Keypoint):
                return np.float32([[point.x, point.y]
                                   for point in coords_input])
            # list of tuples (x, y)
            return np.float32([[point[0], point[1]]
                               for point in coords_input])
        else:
            assert ia.is_np_array(coords_input), (
                "Expected exterior to be a list of tuples (x, y) or "
                "an (N, 2) array, got type %s" % (type(coords_input),))
            assert coords_input.ndim == 2 and coords_input.shape[1] == 2, (
                "Expected exterior to be a list of tuples (x, y) or "
                "an (N, 2) array, got an array of shape %s" % (
                    coords_input.shape,))
            return np.float32(coords_input)
    ...
    def find_closest_point_index(self, x, y, return_distance=False):
        ...
        assert len(self.exterior) > 0, (
            ...
            "contains no points.")
        # Simplified: numpy can do this directly without a loop
        target_point = np.array([x, y], dtype=np.float32)
        distances_sq = np.sum((self.exterior - target_point)**2, axis=1)
        closest_idx = np.argmin(distances_sq)
        if return_distance:
            return closest_idx, np.sqrt(distances_sq[closest_idx])
        return closest_idx
    ...
    def clip_out_of_image(self, image):
        ...
        polygons = self._clip_to_image_bounds_shapely(image)

        # Shapely changes the order of points, we try here to preserve it as
        # much as possible.
        polygons_reordered = self._reorder_polygon_points_by_proximity(
            polygons, self.exterior)

        return polygons_reordered

    def _clip_to_image_bounds_shapely(self, image):
        """Helper to perform the actual shapely-based clipping."""
        ...
        # Handle various shapely intersection results
        if isinstance(multipoly_inter_shapely, shapely.geometry.Polygon):
            ...
            return []
        elif not isinstance(multipoly_inter_shapely,
    ...
    def _reorder_polygon_points_by_proximity(self, polygons, reference_exterior):
        """Helper to reorder points of clipped polygons based on proximity to reference."""
        polygons_reordered = []
        for polygon in polygons:
            best_idx = None
            best_dist = None
            for x, y in reference_exterior:
            ...
            if best_idx is not None:
                ...
                # Should not happen if `polygons` is not empty, but for safety
                polygons_reordered.append(polygon)
        return polygons_reordered
    ...
    def draw_on_image(self,
        ...
        draw_params = self._normalize_drawing_params(
            image, color, color_face, color_lines, color_points,
            alpha, alpha_face, alpha_lines, alpha_points,
            size, size_lines, size_points
        )

        input_dtype = image.dtype
        result = image.astype(np.float32)

        if draw_params["alpha_face"] > 0:
            result = self._draw_filled_polygon(
                result, draw_params["color_face"], draw_params["alpha_face"])

        ls_open = self.to_line_string(closed=False)
        ls_closed = self.to_line_string(closed=True)
        result = ls_closed.draw_lines_on_image(
            result, color=draw_params["color_lines"], alpha=draw_params["alpha_lines"],
            size=draw_params["size_lines"], raise_if_out_of_image=raise_if_out_of_image)
        result = ls_open.draw_points_on_image(
            result, color=draw_params["color_points"], alpha=draw_params["alpha_points"],
            size=draw_params["size_points"], raise_if_out_of_image=raise_if_out_of_image)
    ...
    def _normalize_drawing_params(self, image,
                                  color, color_face, color_lines, color_points,
                                  alpha, alpha_face, alpha_lines, alpha_points,
                                  size, size_lines, size_points):
        """Helper to normalize drawing parameters."""
        def _default_to(var, default):
            if var is None:
                return default
            return var

        assert color is not None, "Expected 'color' to not be None."
        assert alpha is not None, "Expected 'alpha' to not be None."
        assert size is not None, "Expected 'size' to not be None."

        color_face_val = _default_to(color_face, np.array(color))
        color_lines_val = _default_to(color_lines, np.array(color) * 0.5)
        color_points_val = _default_to(color_points, np.array(color) * 0.5)

        alpha_face_val = _default_to(alpha_face, alpha * 0.5)
        alpha_lines_val = _default_to(alpha_lines, alpha)
        alpha_points_val = _default_to(alpha_points, alpha)

        size_lines_val = _default_to(size_lines, size)
        size_points_val = _default_to(size_points, size * 3)

        if image.ndim == 2:
            assert ia.is_single_number(color_face_val), (
                "Got a 2D image. Expected then 'color_face' to be a single "
                "number, but got %s." % (str(color_face_val),))
            color_face_val = [color_face_val]
            color_lines_val = [color_lines_val] if ia.is_single_number(color_lines_val) else color_lines_val
            color_points_val = [color_points_val] if ia.is_single_number(color_points_val) else color_points_val
        elif image.ndim == 3 and ia.is_single_number(color_face_val):
            color_face_val = [color_face_val] * image.shape[-1]
            color_lines_val = [color_lines_val] * image.shape[-1] if ia.is_single_number(color_lines_val) else color_lines_val
            color_points_val = [color_points_val] * image.shape[-1] if ia.is_single_number(color_points_val) else color_points_val


        # Clamp alpha values for drawing logic
        if alpha_face_val < 0.01:
            alpha_face_val = 0
        elif alpha_face_val > 0.99:
            alpha_face_val = 1

        if alpha_lines_val < 0.01:
            alpha_lines_val = 0
        elif alpha_lines_val > 0.99:
            alpha_lines_val = 1

        if alpha_points_val < 0.01:
            alpha_points_val = 0
        elif alpha_points_val > 0.99:
            alpha_points_val = 1

        return {
            "color_face": color_face_val, "color_lines": color_lines_val, "color_points": color_points_val,
            "alpha_face": alpha_face_val, "alpha_lines": alpha_lines_val, "alpha_points": alpha_points_val,
            "size_lines": size_lines_val, "size_points": size_points_val
        }

    def _draw_filled_polygon(self, image_np, color_face, alpha_face):
        """Helper to draw the filled polygon area on an image."""
        rr, cc = skimage.draw.polygon(
            self.yy_int, self.xx_int, shape=image_np.shape)
        if len(rr) > 0:
            if alpha_face == 1:
                image_np[rr, cc] = np.float32(color_face)
            elif alpha_face > 0: # already handled alpha_face == 0
                image_np[rr, cc] = (
                    (1 - alpha_face) * image_np[rr, cc, :]
    ...
    def extract_from_image(self, image):
        ...
        height_mask = np.max(yy_mask) + 1 # +1 for inclusivity
        width_mask = np.max(xx_mask) + 1 # +1 for inclusivity
    ...
    def exterior_almost_equals(self, other, max_distance=1e-4,
        ...
        """
        # Use helper for normalizing 'other'
        other_polygon = self._normalize_input_coords_to_polygon(other)

        return self.to_line_string(closed=True).coords_almost_equals(
            other_polygon.to_line_string(closed=True),
            max_distance=max_distance,
            points_per_edge=points_per_edge
        )

    def _normalize_input_coords_to_polygon(self, other_input):
        """Helper to convert various 'other' inputs to a Polygon instance."""
        if isinstance(other_input, list) or ia.is_np_array(other_input):
            return Polygon(self._normalize_input_coords(other_input))
        assert isinstance(other_input, Polygon), (
            "Expected 'other' to be a list of coordinates, a coordinate "
            "array or a single Polygon. Got type %s." % (type(other_input),))
        return other_input
...
class _ConcavePolygonRecoverer(object):
    ...
    def recover_from(self, new_coords, original_polygon, random_state=0):
        assert isinstance(new_coords, list) or (
            ia.is_np_array(new_coords)
            and new_coords.ndim == 2
            and new_coords.shape[1] == 2), (
                "Expected new_coords as list or (N,2) ndarray, got type %s." % (
                    type(new_coords),))
        ...
        # immediately
        polygon = original_polygon.deepcopy(exterior=new_coords)
        if polygon.is_valid:
            return polygon
        ...
        current_coords = self._remove_consecutive_duplicate_points(new_coords)

        # check that points are not all identical or on a line
        current_coords = self._fix_polygon_is_line(current_coords, rss[0])

        # jitter duplicate points
        current_coords = self._jitter_duplicate_points(current_coords, rss[1])

        # generate intersection points
        segment_add_points = self._generate_intersection_points(
            current_coords, decimals=self.decimals)
        ...
        if self.oversampling is not None and self.oversampling > 0:
            segment_add_points = self._oversample_intersection_points(
                current_coords, segment_add_points)

        # integrate new points into exterior
        coords_with_intersections = self._insert_intersection_points(
            current_coords, segment_add_points)

        # find best fit polygon, starting from convext polygon
        concave_point_indices = self._fit_best_valid_polygon(
            coords_with_intersections, rss[2])
        recovered_coords = [
            coords_with_intersections[idx] for idx in concave_point_indices]

        # TODO return recovered_coords here instead of polygon, leave it to
        #      caller to decide what to do with it
        return original_polygon.deepcopy(exterior=recovered_coords)
    ...
    def _generate_intersection_points(self, exterior,
        ...
        segments = self._create_segments_for_intersection(exterior, decimals)
        ...
        segment_add_points = self._map_intersections_to_segments(
            intersections, segments, one_point_per_intersection)
        ...
        for idx in range(len(segment_add_points)):
            points_and_dists = segment_add_points[idx]
            if len(points_and_dists) < 2:
                segment_add_points_sorted.append([t[0] for t in points_and_dists])
            else:
                both = sorted(points_and_dists, key=lambda t: t[1])
                segment_add_points_sorted.append([a for a, _b in both])
        return segment_add_points_sorted

    def _create_segments_for_intersection(self, exterior, decimals):
        """Helper to create segments from exterior points."""
        segments = []
        for i in range(len(exterior)):
            p0 = exterior[i]
            p1 = exterior[(i + 1) % len(exterior)]
            segments.append(
                (
                    (np.round(float(p0[0]), decimals), np.round(float(p0[1]), decimals)),
                    (np.round(float(p1[0]), decimals), np.round(float(p1[1]), decimals))
                )
            )
        return segments

    def _map_intersections_to_segments(self, intersections, segments, one_point_per_intersection):
        """Helper to map intersection points to their respective segments."""
        ...
        for point, associated_segments in intersections:
            ...
            for seg_inter_p0, seg_inter_p1 in associated_segments:
                diffs = []
                dists_from_segment_start = []
                for seg_p0, seg_p1 in segments:
                    # Compare segment coordinates to find the matching original segment
                    dist_p0p0 = np.linalg.norm(np.array(seg_p0) - np.array(seg_inter_p0))
                    dist_p1p1 = np.linalg.norm(np.array(seg_p1) - np.array(seg_inter_p1))
                    dist_p0p1 = np.linalg.norm(np.array(seg_p0) - np.array(seg_inter_p1))
                    dist_p1p0 = np.linalg.norm(np.array(seg_p1) - np.array(seg_inter_p0))
                    diff = min(dist_p0p0 + dist_p1p1, dist_p0p1 + dist_p1p0)
                    diffs.append(diff)
                    dists_from_segment_start.append(np.linalg.norm(
                        np.array(seg_p0) - np.array(point)
                    ))

                min_diff = np.min(diffs)
                if min_diff < self.max_segment_difference:
                    idx = int(np.argmin(diffs))
                    segments_add_points[idx].append((point, dists_from_segment_start[idx]))
                else:
                    ia.warn(
                        "Couldn't find fitting segment in "
                        "_map_intersections_to_segments(). Ignoring "
                        "intersection point.")
        return segments_add_points
    ...
    def _oversample_intersection_points(self, exterior, segment_add_points):
        ...
        for i, last in enumerate(exterior):
            for j, p_inter_tuple in enumerate(segment_add_points[i]):
                p_inter = p_inter_tuple[0] # intersection point, ignore distance for now
                ...
                for dist_val in oversample: # Renamed 'dist' to 'dist_val' to avoid conflict with outer 'dist' in _map_intersections_to_segments
                    point_over = (last[0] + dist_val * direction[0],
                                  last[1] + dist_val * direction[1])
    ...
    def _fit_best_valid_polygon(self, points, random_state):
        ...
        def _compute_distance_point_to_line(point_coords, line_start_coords, line_end_coords):
            x_diff = line_end_coords[0] - line_start_coords[0]
            y_diff = line_end_coords[1] - line_start_coords[1]
            num = abs(
                y_diff*point_coords[0] - x_diff*point_coords[1]
                + line_end_coords[0]*line_start_coords[1] - line_end_coords[1]*line_start_coords[0]
            )
            den = np.sqrt(y_diff**2 + x_diff**2)
            if den == 0:
                return np.sqrt(
                    (point_coords[0] - line_start_coords[0])**2
                    + (point_coords[1] - line_start_coords[1])**2)
            return num / den
        ...
        current_kept_indices = list(hull.vertices)
        remaining_indices = [i for i in range(len(points)) if i not in current_kept_indices]

        iteration = 0
        n_changes = 0
        converged = False
        while not converged:
            candidates = self._generate_candidate_edges(
                points, current_kept_indices, remaining_indices, iteration, random_state,
            ...
            # Iterate over point-segment pairs in sorted order.
            ...
            for candidate_idx in candidate_ids:
                ...
                # Find position of point_kept_idx in the *current* points_kept list
                try:
                    in_points_kept_idx = current_kept_indices.index(point_kept_idx)
                except ValueError:
                    # point_kept_idx might have been removed if a previous candidate
                    # modified the list. This is problematic if multiple additions
                    # are expected per outer iteration. The original code's `is_valid` flag
                    # structure suggests that each successful addition `modifies` `points_kept`
                    # and `points_left` for subsequent candidate considerations *within the same outer iteration*.
                    # Let's handle this by re-calculating position of `point_kept_idx`.
                    # If it's no longer there, this candidate is invalid for the current list.
                    continue

                points_kept_hypothesis = current_kept_indices[:]
                ...
                if poly_hypothesis.is_valid:
                    ...
                    current_kept_indices = points_kept_hypothesis
                    # Remove from remaining_indices list as it's now 'kept'
                    remaining_indices.remove(point_left_idx)
                    n_changes += 1
                    if n_changes >= self.fit_n_changes_max:
                        return current_kept_indices
                
            if not found_valid_change and iteration > 0:
                converged = True
    ...
    def _generate_candidate_edges(self, all_points, current_kept_indices, remaining_indices, iteration, random_state, _compute_distance_point_to_line):
        """Helper to generate and filter candidate edges for polygon fitting."""
        candidate_data = []
        point_kept_idx_to_pos = {
            point_idx: i for i, point_idx in enumerate(current_kept_indices)}

        if len(remaining_indices) == 0 or len(current_kept_indices) == 0:
            return []

        # Generate all possible combinations from <current_kept_indices> and <remaining_indices>
        for point_left_idx in remaining_indices:
            for point_kept_idx in current_kept_indices:
                # Calculate vertex distance
                p_idx_min = min(point_left_idx, point_kept_idx)
                p_idx_max = max(point_left_idx, point_kept_idx)
                vertex_dist = np.minimum(
                    p_idx_max - p_idx_min,
                    len(all_points) - p_idx_max + p_idx_min
                )

                # limit candidates by vertex_dist threshold
                max_dist_threshold = self.fit_max_dist_other_iters
                if iteration == 0:
                    max_dist_threshold = self.fit_max_dist_first_iter
                
                if vertex_dist <= max_dist_threshold:
                    # Find the segment (edge) in `current_kept_indices` that `point_kept_idx` is a start of.
                    # This relies on `current_kept_indices` maintaining a polygon-like order.
                    in_points_kept_pos = point_kept_idx_to_pos[point_kept_idx]
                    segment_start_idx = point_kept_idx
                    segment_end_idx = current_kept_indices[
                        (in_points_kept_pos + 1) % len(current_kept_indices)]
                    segment_start_coords = all_points[segment_start_idx]
                    segment_end_coords = all_points[segment_end_idx]

                    dist_eucl = 0
                    if iteration > 0: # Only compute euclidean distance after first iteration for efficiency
                        dist_eucl = _compute_distance_point_to_line(
                            all_points[point_left_idx], segment_start_coords, segment_end_coords)
                    
                    candidate_data.append(
                        (point_left_idx, point_kept_idx, vertex_dist, dist_eucl)
                    )

        # Apply random subsampling if too many candidates
        if (self.fit_n_candidates_before_sort_max is not None
                and len(candidate_data) > self.fit_n_candidates_before_sort_max):
            random_state.shuffle(candidate_data) # Shuffle in place
            candidate_data = candidate_data[0:self.fit_n_candidates_before_sort_max]
