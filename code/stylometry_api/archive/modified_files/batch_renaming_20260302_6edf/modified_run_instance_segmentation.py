...
class Evaluator:
    ...
    def postprocess_prediction_batch(self, prediction_batch, target_sizes):
        results = []
        for pred, size in zip(prediction_batch, target_sizes):
            if pred:
                results.append({
                    "masks": pred[0],
                    "labels": pred[1],
    ...
    def __call__(self, evaluation_results: EvalPrediction, compute_result: bool = False):
        ...
        post_processed_targets = self.postprocess_target_batch(targets)
        target_sizes = self.get_target_sizes(post_processed_targets)
        post_processed_predictions = self.postprocess_prediction_batch(predictions, target_sizes)
