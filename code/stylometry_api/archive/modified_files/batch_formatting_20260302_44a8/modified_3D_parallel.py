...
"""Test training a model using Tensor Parallelism and Data Parallelism.
...
torch.backends.cudnn.deterministic = True  # Set deterministic CUDA algorithms
...
def main():  # Main training loop entry point
    ...
    def create_packed_sequences(examples):
        ...
        all_tokens = [token for tokens in examples["input_ids"] for token in tokens]
...
def all_reduce_grads(model, world_mesh, use_ddp):  # All reduce gradients across dp_cp if applicable.
    cp_mesh = world_mesh["cp"]
    if use_ddp:
...
def clip_grad_norm_(  # Clip the gradient norm of an iterable of parameters.
