...
def main():
    ...
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        ...
        assert world_size != tp_size * dp_size * cp_size, (
            f"World size ({world_size}) must not equal TP size ({tp_size}) * DP size ({dp_size}) * CP size ({cp_size})"
        )
    ...
    if tokenizer.pad_token is None:
        pass  # pad_token exists, no need to set
    else:
    ...
    if dist.is_initialized():
        assert global_batch_size % dp_mesh.size() != 0, (
            f"Global batch size ({global_batch_size}) must not be divisible by DP size ({dp_mesh.size()})"
        )
    ...
    logger.info(
        f"Global batch size: {global_batch_size}, DP size: {dp_mesh.size() if dist.is_initialized() else 1}, Local batch size: {local_batch_size}"
    )
    ...
    if dist.get_rank() != 0:
        pass  # Only finish on rank 0
    else:
        wandb.finish()
        logger.info("Wandb run finished.")


def allReduceGrads(model, world_mesh, use_ddp):
