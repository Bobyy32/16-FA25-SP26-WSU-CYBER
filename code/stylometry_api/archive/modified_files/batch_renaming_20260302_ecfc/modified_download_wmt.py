...
def download_wmt_dataset(src_lang="ro", tgt_lang="en", dataset="wmt16", save_dir=None) -> None:
    ...
    for split in ds:
        ...
        # Determine file prefix for validation vs training
        prefix = "val" if split == "validation" else split
        src_path = save_dir.joinpath(f"{prefix}.source")
        tgt_path = save_dir.joinpath(f"{prefix}.target")
        src_fp = src_path.open("w+")
        tgt_fp = tgt_path.open("w+")

        # Efficient batch processing using list comprehension
        _ = [src_fp.write(x["translation"][src_lang] + "\n") or tgt_fp.write(x["translation"][tgt_lang] + "\n") 
