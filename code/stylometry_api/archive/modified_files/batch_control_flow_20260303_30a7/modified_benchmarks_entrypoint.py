...
#
# Unless required by applicable law or agreed to be in writing, software
...
if __name__ == "__main__":
    ...
    if os.path.exists(benches_folder_path):
        ...
        entries_list = [e for e in reversed(list(os.scandir(benches_folder_path)))]
    ...
    [
        (module_path := os.path.join(benches_folder_path, module_name),
         try_(
            lambda: logger.debug(f"loading: {module_name}"),
            lambda: import_from_path(module_name.split(".")[0], module_path),
            lambda: logger.info(f"running benchmarks in: {module_name}"),

            (try_(
                lambda: module.run_benchmark(logger, repository, branch, commit_id, commit_msg, global_metrics_recorder),
                lambda: logger.warning(
                    f"Module {module_name} using old run_benchmark signature - database connection will be created per module"
                ),
                lambda: module.run_benchmark(logger, repository, branch, commit_id, commit_msg)
            ) or True),
            lambda: successful_benchmarks += 1,
        ))
