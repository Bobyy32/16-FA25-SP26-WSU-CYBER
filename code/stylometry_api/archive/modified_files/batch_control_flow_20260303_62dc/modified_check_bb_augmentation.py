...
def main():
    ...
    kp_list = []
    for yrow in range(NB_ROWS):
        ycoord = BB_Y1 + int(yrow * (BB_Y2 - BB_Y1) / (NB_COLS - 1))
        for xcol in range(NB_COLS):
            xcoord = BB_X1 + int(xcol * (BB_X2 - BB_X1) / (NB_ROWS - 1))
            kp = (xcoord, ycoord)
            kp_list.append(kp)
    kp_set = set(kp_list)
    kps = [ia.Keypoint(x=xval, y=yval) for (xval, yval) in kp_set]
